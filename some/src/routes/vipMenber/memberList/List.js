...
const List = ({
  onDeleteItem, onEditItem, onEditItemWatch,isMotion, location, ...tableProps
}) => {
  ...
  const columns = [
  ...
  
    {
      ...
      render: (text, record) => {
        ...
        return   <div>
            <span style={{cursor:'pointer',color:'#1890ff'}} onClick={e => onEditItem(record)} >查看详情</span>
            <br/>
            <span style={{cursor:'pointer',color:'#1890ff'}} onClick={e => onEditItemWatch(record)} >查看账目明细</span>
          </div>
      },
    },
  ]
...
List.propTypes = {
  ...
  onEditItemWatch: PropTypes.func,
