...
const User = ({
  location, dispatch, articleClassify, loading,
}) => {
  ...
  const modalProps = {
    ...
    confirmLoading: loading.effects[`articleClassify/${modalType}`],
    ...
    onOk (data) {
      ...
      if(modalType=='update'){
        dispatch({
          type: `articleClassify/${modalType}`,
          payload: data,
        })
          .then(() => {
            handleRefresh()
    ...
    onCancel () {
      dispatch({
        type: 'articleClassify/hideModal',
      })
    },
  }
  ...
  const listProps = {
    dataSource: list,
    loading: loading.effects['articleClassify/query'],//暂时理解为触发modal的事件名称,拿数据用的
    pagination,
    location,
    isMotion,
    onChange (page) {
    ...
    onEditItem (item) {
      dispatch({
        type: 'articleClassify/showModal',
        payload: {
          modalType: 'update',
          currentItem: item,
        },
    ...
    rowSelection: {
      ...
      onChange: (keys) => {
        dispatch({
          type: 'articleClassify/updateState',
          payload: {
  ...
  const filterProps = {
    ...
    switchIsMotion () {
      dispatch({ type: 'articleClassify/switchIsMotion' })
    },
  }
...
User.propTypes = {
  articleClassify: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
  loading: PropTypes.object,
}

export default connect(({ articleClassify, loading }) => ({ articleClassify, loading }))(User)
